...
{
    // Convert Boodle:// to Boodle:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Boodle://"))
    {
        uri.replace(0, 12, "Boodle:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Boodle.lnk";
}
...
{
    return GetAutostartDir() / "Boodle.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Boodle-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Boodle-qt [" + tr("command-line options") + "]                     " + "\n";
